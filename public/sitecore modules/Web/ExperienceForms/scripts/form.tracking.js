(function($jq) {
	...
	function getOwner(form, elementId) {
		var targetId =
			elementId.slice(
				0,
				-(elementId.length - elementId.lastIndexOf('.') - 1)
			) + 'Value';
	...
	function getSessionId(form) {
		...
		var targetId =
			formId.slice(0, -(formId.length - formId.lastIndexOf('_') - 1)) +
			'FormSessionId';
	...
	function getElementName(element) {
		...
		if (!endsWith(fieldName, 'value')) {
			...
			return (
				fieldName.substring(0, index + searchPattern.length + 3) +
				'Value'
			);
		}
	...
	($jq.fxbFormTracker = function(el, options) {
		...
		this.options = $jq.extend(
			{},
			$jq.fxbFormTracker.defaultOptions,
			options
		);
		this.init();
	}),
			...
			prototype: {
				...
				startTracking: function() {
					...
					if (trackedInputs.length) {
						inputs.not(trackedInputs).bind('focus', function() {
				...
				onBlurField: function(element) {
					...
					var duration = this.options.duration
						? Math.round((timeStamp - this.options.duration) / 1000)
						: 0;
					...
					if (fieldChanged || this.options.fieldValue !== value) {
						...
						var clientEvent = this.buildEvent(
							fieldId,
							fieldName,
							eventIds.fieldCompleted,
							duration
						);
						...
						if (validator && !validator.element(element)) {
							validationEvents = this.checkClientValidation(
								element,
								fieldName,
								validator,
								duration
						...
						this.trackEvents(
							$jq.merge([clientEvent], validationEvents)
						);
					}
				},
				...
				checkClientValidation: function(
					element,
					fieldName,
					validator,
					duration
					...
					$jq.each(validator.errorMap, function(key) {
						if (key === element.name) {
							var clientEvent = tracker.buildEvent(
								key,
								fieldName,
								eventIds.fieldError,
								duration
