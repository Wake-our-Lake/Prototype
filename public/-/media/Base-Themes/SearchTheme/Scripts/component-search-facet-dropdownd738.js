XA.component.search.facet.dropdown = (function ($, document) {
    ...
    var FacetDropdownModel = Backbone.Model.extend({
        defaults: {
        ...
        processData: function (data) {
            var inst = this,
                ...
                sig = inst.get("sig"),
                searchResultsSignature = this.get("dataProperties").searchResultsSignature.split(','),
                facet = inst.get("dataProperties").f,
                facetItem,
                facedData,
                i, j;
            ...
            for (j = 0; j < searchResultsSignature.length; j++) {
                if (data.Facets.length > 0 && (data.Signature === searchResultsSignature[j]) || data.Signature === "" || data.Signature === null) {
                    ...
                    inst.set({ resultData: facedData !== undefined ? facedData.Values : null });
                }
            }
        },
